...
function buildTopicBank(){
  ...
  Object.keys(topics).sort((a,b)=>+a-+b).forEach(n=>{
    ...
    shuffle(facts).slice(0,24).forEach((f,i)=>{
      const others=facts.filter(x=>x!==f&&x.def!==f.def&&x.term!==f.term); if(others.length<3)return;
      const reverse=i%3===1;
      let question,answers;
      if(reverse){
        question=`¿A qué concepto corresponde la siguiente descripción? «${f.def}»`;
        answers=[f.term,...shuffle(others).slice(0,3).map(x=>x.term)];
      }else{
        question=`¿Cuál de las siguientes afirmaciones describe correctamente ${f.term}?`;
        answers=[f.def,...shuffle(others).slice(0,3).map(x=>x.def)];
      }
