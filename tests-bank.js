...
function buildTopicBank(){
  ...
  Object.keys(topics).sort((a,b)=>+a-+b).forEach(n=>{
    ...
    shuffle(facts).slice(0,24).forEach((f,i)=>{
      ...
      const reverse=i%3===1;
      const title=t.title.replace(/^TEMA \d+\.\s*/,'');
      const question=reverse?`¿Qué concepto corresponde a la siguiente descripción en ${title}: «${f.def}»?`:`Según ${title}, ¿qué afirmación corresponde a «${f.term}»?`;
